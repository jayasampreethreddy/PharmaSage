...
async function loadMolecule() {
    ...
    try {
        ...
        if (drugList.includes(drugName)) {
            ...
            response = await fetch(`/api/drug/${encodeURIComponent(drugName)}`);
        } else {
            // Use search endpoint
            response = await fetch(`/api/search_drug?query=${encodeURIComponent(drugName)}`);
        }
        
        const drugData = await response.json();
        
        if (drugData.error || !drugData.drug_name) {
            showError(drugData.error || 'Drug not found. Please check the name or SMILES.');
            showLoading(false);
            return;
        }
        ...
        if (!molData.molblock) {
            showError(molData.error || 'Could not generate molecule structure.');
            showLoading(false);
            return;
        }
        ...
        document.getElementById('main-molecule-name').textContent = drugData.drug_name || '';
        
        // Display properties
        displayProperties(drugData, 'properties-content');
        document.getElementById('properties-card').style.display = 'block';
        
        currentMolecule = drugData;
        
    } catch (error) {
        console.error('Error loading molecule:', error);
        showError('Failed to load molecule data');
    } finally {
        showLoading(false);
    }
}
...
async function compareDrugs() {
    const drug1Name = document.getElementById('drug1-select').value;
    const drug2Name = document.getElementById('drug2-select').value;
    if (!drug1Name || !drug2Name) {
        showError('Please select both drugs for comparison');
        return;
    }
    if (drug1Name === drug2Name) {
...
function renderMolecule(viewer, molblock, drugName) {
    ...
    try {
        ...
        // Set viewer container size for consistency
        if (viewer.container) {
            viewer.container.style.width = '100%';
            viewer.container.style.height = '250px';
        }
    } catch (e) {
        // fallback: do nothing
    }
